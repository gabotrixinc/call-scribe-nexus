
import React from 'react';
import Layout from '@/components/Layout';

const IntegrationsPage: React.FC = () => {
  return (
    <Layout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Integraciones</h1>
            <p className="text-muted-foreground">Conecta tu centro de contacto con otras plataformas y servicios.</p>
          </div>
        </div>
        
        <div className="flex flex-col items-center justify-center h-[60vh] bg-muted/30 rounded-lg border border-dashed">
          <h3 className="text-2xl font-semibold mb-2">Integraciones de Terceros</h3>
          <p className="text-muted-foreground mb-4 text-center max-w-md">
            Esta funcionalidad estará disponible próximamente. Podrás integrar el centro de contacto con múltiples servicios externos.
          </p>
        </div>
      </div>
    </Layout>
  );
};

export default IntegrationsPage;
